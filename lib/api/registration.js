...
routes.get('/confirm', eventDeadlineValidator, (request, response) => {
  ...
  Registrant.findOne({email: userEmail, emailVerified: true}, (err, registrant) => {
    ...
    const emailData = {
      subject: 'What You Need to Know For RevolutionUC!',
      shortDescription: 'We’re excited to have you at RevolutionUC 🙌. Here’s some information to get ready for the event',
    ...
    Registrant.update({email: userEmail}, {attendanceConfirmed: confirmValue}, (err, numberAffected, rawResponse) => {
      ...
      if (confirmValue == 'YES') {
        ...
        build('confirmAttendanceFollowUp', emailData)
          .then(html => {
