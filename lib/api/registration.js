...
/**
 * This is used to send the email in revolutionuc-emails
 * denoted by name `infoEmail1`, to all registrants whose
...
routes.get('/sendInfoEmail1', apiKeyValidator, eventDeadlineValidator, (request, response) => {
  Registrant.find({emailVerified: true}, (err, registrants) => {
    ...
    sendInfoEmail1(request, registrants, {offWaitlist: true});
    response.json({"sendInfoEmail1": "sent"});
  });
});
