...
module.exports = (request, response, next) => {
  ...
  request.checkBody('githubUsername', `GitHub username ${emptyErrorMessage}`).notEmpty();
  request.checkBody('resume', `Resume ${emptyErrorMessage}`).notEmpty();

  const errors = request.validationErrors();

  if (errors) {
    response.status(400).send(util.inspect(errors));
    return;
  }
