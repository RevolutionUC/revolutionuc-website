...
const upload = multer({storage, limits: { fileSize: 20000000, files: 1 }});
...
routes.post('/register', (request, response) => {
  upload.single('resume')(request, response, err => {
    ...
    // Set meta defaults
    const resumePath = RESUME_PATH.slice(RESUME_PATH.length - 1) != '/' ? RESUME_PATH + '/' : RESUME_PATH
...
routes.get('/verify', (request, response) => {
  ...
  Registrant.update({email: userEmail}, {
    emailVerified: true
