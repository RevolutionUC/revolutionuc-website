  ...
  upload.single('resume')(request, response, err => {
    ...
    Registrant.find((err, registrants) => {
      ...
      newRegistrant.save((err, savedRegistrant) => {
        ...
        email.build(emailTemplate, emailData, 'RevolutionUC <info@revolutionuc.com>', savedRegistrant.email)
          .then(email => {
...
routes.get('/sendConfirmation', (request, response) => {
  ...
  Registrant.find((err, allRegistrants) => {
    ...
    for (registrant of allRegistrants) {
      if (registrant.emailVerified && !registrant.receivedAttendanceConfirmationEmail && !registrant.waitList && registrant.attendanceConfirmed === 'NA') {
        console.log(`Sending attendance confirmation email to ${registrant.email}`);
        ...
        emailData.yesConfirmationUrl = `${confirmationUrl}&confirm=YES`,
        emailData.noConfirmationUrl = `${confirmationUrl}&confirm=NO`,
...
routes.get('/confirm', (request, response) => {
  ...
  Registrant.findOne({email: userEmail}, (err, registrant) => {
    ...
    if (registrant && registrant.attendanceConfirmed != 'NA') {
    ...
    Registrant.update({email: userEmail}, {attendanceConfirmed: confirmValue}, (err, numberAffected, rawResponse) => {
      if (err) response.status(400).send('Sorry, something went wrong');
      response.status(200).redirect('/attendance-confirmed');
    });
  });
});
