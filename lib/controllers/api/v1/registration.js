...
routes.get('/sendSubsetConfirmation', (request, response) => {
  if (request.query.key !== API_KEY) return response.json({authenticated: false});

  const arrayOfEmails = [
    'farolidm@mail.uc.edu',
    'bassnh@mail.uc.edu',
    'collinzy@mail.uc.edu'
  ];

  Registrant.find()
    .where('email')
    .in(arrayOfEmails)
    .exec((err, registrants) => {
...
routes.get('/sendConfirmationIndividual', (request, response) => {
  ...
  Registrant.find({email: userEmail}, (err, registrants) => {
    ...
    Registrant.update({email: userEmail}, {waitList: false, receivedAttendanceConfirmationEmail: false}, (err, numberAffected, rawResponse) => {
...
function sendConfirmationEmails(request, registrants) {
  for (let registrant of registrants) {
      if (registrant.emailVerified && !registrant.waitList && !registrant.receivedAttendanceConfirmationEmail) {
