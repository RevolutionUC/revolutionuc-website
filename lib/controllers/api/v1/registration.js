...
routes.get('/sendConfirmationToWaitlisted', (request, response) => {
  if (request.query.key !== API_KEY) return response.json({authenticated: false});

  Registrant.find({emailVerified: true, waitList: true}, (err, registrants) => {
    if (err) return console.error(err);

    Registrant.update({emailVerified: true, waitList: true}, {waitList: false, receivedAttendanceConfirmationEmail: true}, {multi: true}, (err, numberAffected, rawResponse) => {
