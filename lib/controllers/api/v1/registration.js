...
routes.post('/register', (request, response) => {
  upload.single('resume')(request, response, err => {
    ...
    Registrant.find((err, registrants) => {
      ...
      /**
       * Check it user has already registered
       */
      for (let registrant of registrants) {
        console.log(registrant.email);
        if (registrant.email.toLowerCase() == request.body.email.toLowerCase()) return response.status(400).json([{param: 'email', msg: 'Email address has already been registered'}]);
      }
