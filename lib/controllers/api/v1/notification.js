...
const {apiKeyValidator, notificationChannelValidator} = require('./mwValidators');
 ...
 * Endpoints
 */
router.get('/credentials', apiKeyValidator, (request, response, next) => {
  PushCredentials.find((err, allPushCredentials) => {
    response.json(allPushCredentials);
  });
});

router.get('/pushAll', apiKeyValidator, notificationChannelValidator, (request, response, next) => {
  const pushPayload = {
