export class Router {
  ...
  _swapContents() {
    ...
    const newMastHeadTitle = this._newContent.querySelector('div.masthead div.masthead-title');
    ...
    // TODO: Dom, this is a hack (from dom). Consider breaking the idea of a "view" out
    // into its own class so we can more semantically differentiate views with and without
    // mastheads. This will also make it easier to construct more types of views = more flexible site.
    if (this._mastHead && newMastHead) {
    ...
    } else if (this._mastHead) {
      this._viewElement.removeChild(this._mastHead);
      this._mastHead = null;
    } else if (newMastHead) {
      console.log('actually adding masthead');
      this._mastHead = newMastHead;
      this._viewElement.insertBefore(this._mastHead, this._viewElement.firstChild);
    }
  ...
  _bootstrapBodyNavLinks() {
    Array.prototype.forEach.call(document.querySelectorAll('div[view] *[router-link]'), link => {
      ...
      // first child in a div/p with any whitespace is
      // the #text node. Use css selector intead
      const firstChild = link.querySelector(':first-child');
